    ...
    <div class="p-4 sm:p-6 space-y-6" x-data="{
        ...
        @if (session('success'))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
                ...
                class="fixed top-5 right-5 bg-green-600 text-white px-6 py-3 rounded-xl shadow-lg z-[60] flex items-center gap-3">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-green-700">Edit FAQ</h1>
                <p class="text-sm text-gray-500 mt-1">Perbarui pertanyaan dan jawaban FAQ.</p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.faq.update', $faq) }}" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <textarea name="question" rows="3" required
                        ...
                        placeholder="Contoh: Bagaimana cara memesan pupuk organik?">{{ old('question', $faq->question) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="answer" rows="6" required
                        class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 focus:bg-white outline-none transition-all duration-300 resize-none"
                        placeholder="Tulis jawaban lengkap untuk pertanyaan di atas...">{{ old('answer', $faq->answer) }}</textarea>
                </div>
                    ...
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <div class="relative">
                            <input type="checkbox" name="is_verified" value="1"
                                {{ old('is_verified', $faq->is_verified) ? 'checked' : '' }} class="sr-only peer">
