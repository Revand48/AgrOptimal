...
@section('title', 'Edit Tips Irigasi')
...
@section('content')
    <div class="p-4 sm:p-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-green-700">Edit Tips Irigasi</h1>
                <p class="text-sm text-gray-500 mt-1">Perbarui informasi tips irigasi.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-green-100/80 overflow-hidden max-w-4xl relative">
            ...
            <form action="{{ route('admin.edukasi.irigasi.update', $tip->id) }}" method="POST"
                ...
                <div>
                    ...
                    @if ($tip->image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $tip->image) }}" class="h-20 w-20 object-cover rounded-lg">
                        </div>
                    @endif
                    ...
                    <div x-data="{ tab: 'padi' }" class="bg-gray-50 p-4 rounded-xl border border-gray-200">
                        ...
                        <div x-show="tab === 'padi'" x-transition:enter="transition ease-out duration-200"
                            ...
                            <textarea name="content_padi" rows="5"
                                ...
                                placeholder="Tips khusus untuk Padi...">{{ old('content_padi', $tip->content_padi) }}</textarea>
                        </div>
                        <div x-show="tab === 'jagung'" style="display: none;"
                            ...
                            <textarea name="content_jagung" rows="5"
                                class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none"
                                placeholder="Tips khusus untuk Jagung...">{{ old('content_jagung', $tip->content_jagung) }}</textarea>
                        </div>
                        <div x-show="tab === 'kedelai'" style="display: none;"
                            ...
                            <textarea name="content_kedelai" rows="5"
                                class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none"
                                placeholder="Tips khusus untuk Kedelai...">{{ old('content_kedelai', $tip->content_kedelai) }}</textarea>
                        </div>
                        <div x-show="tab === 'singkong'" style="display: none;"
                            ...
                            <textarea name="content_singkong" rows="5"
                                class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none"
                                placeholder="Tips khusus untuk Singkong...">{{ old('content_singkong', $tip->content_singkong) }}</textarea>
                        </div>
                        <div x-show="tab === 'ubi'" style="display: none;"
                            ...
                            <textarea name="content_ubi" rows="5"
                                class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none"
                                placeholder="Tips khusus untuk Ubi...">{{ old('content_ubi', $tip->content_ubi) }}</textarea>
                        </div>
                    </div>
                </div>
