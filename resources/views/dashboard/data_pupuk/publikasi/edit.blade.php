...
@section('content')
    <div class="p-4 sm:p-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-green-700">Edit Publikasi</h1>
                <p class="text-sm text-gray-500 mt-1">Perbarui data publikasi pupuk.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-green-100/80 overflow-hidden max-w-2xl relative">
            ...
            <form action="/dashboard/data_pupuk/publikasi/{{ $publikasi->id }}" method="POST"
                        ...
                        <select name="desa_id" required
                            ...
                            @foreach ($desas as $desa)
                                <option value="{{ $desa->id }}"
                                    {{ $publikasi->desa_id == $desa->id ? 'selected' : '' }}>
                                    {{ $desa->nama }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="pupuk_id" required
                            ...
                            @foreach ($pupuks as $pupuk)
                                <option value="{{ $pupuk->id }}"
                                    {{ $publikasi->pupuk_id == $pupuk->id ? 'selected' : '' }}>
                                    {{ $pupuk->nama }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="jumlah_sak" value="{{ $publikasi->jumlah_sak }}" required
                            placeholder="0" min="1"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="is_publish"
                                class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 focus:bg-white outline-none transition-all duration-300 appearance-none">
                                <option value="1" {{ $publikasi->is_publish ? 'selected' : '' }}>Publish</option>
                                <option value="0" {{ !$publikasi->is_publish ? 'selected' : '' }}>Draft</option>
                            </select>
                            <div
