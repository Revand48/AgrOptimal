...
@section('title', 'Publikasi Data Pupuk')
...
@section('content')
    ...
    @endif">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-green-700">Publikasi Data Pupuk</h1>
                <p class="text-sm text-gray-500 mt-1">Kelola data pupuk yang akan dipublikasikan ke masyarakat.</p>
            </div>
        </div>
            ...
            <div class="p-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-lg font-bold text-gray-800">Daftar Publikasi</h2>
                            <p class="text-xs text-gray-500">Semua data publikasi tersimpan.</p>
                        </div>
                    </div>
                    <a href="/dashboard/data_pupuk/publikasi/create"
                        ...
                        <tbody class="divide-y divide-green-50">
                            @forelse($publikasis as $item)
                                <tr class="hover:bg-green-50/50 transition-colors">
                                    <td class="p-4 text-gray-700 font-medium">{{ $item->desa->kecamatan->nama }}</td>
                                    <td class="p-4 text-gray-700">{{ $item->desa->nama }}</td>
                                    <td class="p-4 text-gray-700">{{ $item->pupuk->nama }}</td>
                                    ...
                                    <td class="p-4 text-center font-semibold text-gray-800">
                                        {{ number_format($item->jumlah_sak * $item->pupuk->kg_per_sak, 0, ',', '.') }} kg
                                    </td>
                                    <td class="p-4 text-center">
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $item->is_publish ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-gray-100 text-gray-800 border border-gray-200' }}">
                                            <span
                                                class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $item->is_publish ? 'bg-green-500' : 'bg-gray-500' }}"></span>
                                            {{ $item->is_publish ? 'Publish' : 'Draft' }}
                                        </span>
                                    </td>
                                    <td class="p-4 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="/dashboard/data_pupuk/publikasi/{{ $item->id }}/edit"
                                            ...
                                            <button type="button"
                                                @click="openDeleteModal('/dashboard/data_pupuk/publikasi/{{ $item->id }}', 'Publikasi {{ $item->desa->nama }}')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="p-8 text-center text-gray-500">
                                        ...
                                        <p class="font-medium">Belum ada data publikasi</p>
                                        <p class="text-sm mt-1">Klik tombol "Tambah Publikasi" untuk menambah data baru.
                                        </p>
                                    </td>
                                </tr>
                            @endforelse
